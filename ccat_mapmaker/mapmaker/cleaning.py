...
def remove_cosmic_rays(tod_1d: np.ndarray,
                       sigma: float = 3.5,
                       n: int = 2) -> np.ndarray:
    """
    ...
    dtod = np.diff(tod, append=tod[-1])

    threshold    = np.std(tod) * sigma
...
def clean_tod(tod: np.ndarray,
    ...
    if cosmic_rays:
        for i in range(n_dets):
            tod_clean[:, i] = remove_cosmic_rays(tod_clean[:, i])

    if highpass_hz > 0.0:
        tod_fft = fft.rfft(tod_clean, axis=0)
