...
def _load_simulation_focalplane(frame):
    """
...
# Adapted from Bonnie Slocombe, https://github.com/bonnieslocombe/g3_mapmaking, mapmaker/g3mapmaker.py, QuickMapMaker.Process
def _simulation_scan_to_chunk(frame, det_names, det_dirs, sample_rate_ref):
    """
...
def _iter_simulation_chunks(g3_files) -> Iterator[Chunk]:
    """Yield one frame-level Chunk per scan frame from CCAT simulation .g3 files."""
    det_names       = None
    det_dirs        = None
    ...
    for path in g3_files:
        for frame in core.G3File(str(path)):
            if frame.type == core.G3FrameType.Calibration:
                det_names, _, det_dirs = _load_simulation_focalplane(frame)
                ...
                yield _simulation_scan_to_chunk(
                    frame, det_names, det_dirs, sample_rate_ref
                )
...
def iter_g3_chunks(cfg: dict) -> Iterator[Chunk]:
    ...
    if not g3_files:
        raise FileNotFoundError(
            f"No .g3 files found for patterns: {cfg['data']['input_dirs']}"
        )

    fmt = cfg["data"]["format"]
    if fmt == "simulation":
        frame_iter = _iter_simulation_chunks(g3_files)
    else:
    ...
    t_obs_start = None
    for chunk in _rechunk(frame_iter, chunk_duration_s):
